...
#
# Covers every script whose shebang is /bin/sh, plus the connection script
# generated by host-connect-setup.sh: that is the tier 1 file, the one that has
# to run under busybox ash inside a guest container.
#
...
# 3. Check the generated guest bundle, which no static scan of the repository
# would otherwise reach. A throwaway key keeps this independent of whether the
# machine has readable host keys.
